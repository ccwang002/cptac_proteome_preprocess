---
title: "Create UniProt entry mapping between RefSeq 20111201 and UniProt 2020_03"
output:
    html_document:
        df_print: paged
---
...
Source here is RefSeq 20111201 and Target is UniProt 2020_03
```{r}
source_annotation_tbl = read_tsv(here('annotation/refseq_20111201/tracked_results/refseq_20111201_human_only.tsv.gz'))
...
mapping_tbl.align_by_symbol = source_annotation_tbl.remaining %>%
    select(refseq_prot_id, hgnc_id, symbol=hgnc_symbol, source_uniparc_id = uniparc_id) %>%
    inner_join(
        target_tbl %>% 
...
```{r}
mapping_tbl %>% 
    write_tsv('../tracked_results/mappings/refseq_20111201_to_uniprot_2020_03_mappable.tsv.gz')

source_annotation_tbl.failed %>%
    write_tsv('../tracked_results/mappings/refseq_20111201_to_uniprot_2020_03_not_mappable.tsv.gz')
```
