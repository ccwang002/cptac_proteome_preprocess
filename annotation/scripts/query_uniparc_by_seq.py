...
async def query_uniparc(
    prot_id: str, prot_seq: str,
    session: aiohttp.ClientSession, json_out_pth: Path
):
    if json_out_pth.exists():
        logger.info(f'... skip {prot_id} query because its JSON exists')
        j = json.load(json_out_pth.open('rt'))
    ...
    json_out_pth.write_text(t)

    j = await resp.json()
    return parse_uniparc_json(prot_id, j)


async def main(prot_ids: List[str], protein_fa: FastaFile, out_tsv_pth: Path, json_root: Path):
    ...
    if ids_without_seq:
        ...
        raise ValueError(f'Cannot find the sequence of {len(ids_without_seq)} IDs in FASTA')
    ...
    async with session:
        ...
        for prot_id in prot_ids:
            ...
            json_out_pth = Path(json_dir, f'{prot_id}.json')
            tasks.append(asyncio.create_task(
                query_uniparc(prot_id, prot_seq, session, json_out_pth),
                name=prot_id
            ))

        for i, res in range(asyncio.as_completed(tasks), 1):
            record = await res
            if record is None:
                ids_without_records.append(res.get_name())
                continue
...
def setup_cli():
    ...
    parser.add_argument('--json-dir', help="Folder to store the raw query JSONs",
                        default='.')
