...
mapping_tbl.identical = source_annotation_tbl.remaining %>%
    select(ensembl_prot_id, uniq_protein_seq_entry_no, symbol, hgnc_id, source_uniparc_id = uniparc_id) %>%
...
mapping_tbl.align = source_annotation_tbl.remaining %>%
    select(ensembl_prot_id, symbol, hgnc_id, source_uniparc_id = uniparc_id) %>%
    inner_join(
        target_tbl %>% 
...
mapping_tbl.align_by_symbol = source_annotation_tbl.remaining %>%
    select(ensembl_prot_id, hgnc_id, symbol, source_uniparc_id = uniparc_id) %>%
    inner_join(
        target_tbl %>% 
