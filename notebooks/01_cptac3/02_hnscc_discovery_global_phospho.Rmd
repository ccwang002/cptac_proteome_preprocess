...
tmt_map_clean_tbl = tmt_map_tbl %>%
    ...
    pivot_longer(-folder, names_to = c('tmt_plex', 'id_type'), names_pattern = '^TMT11-(\\d+[NC]?) (.*)$') %>%
...
protein_sample_tbl = bind_cols(
    ...
    tmt_map_clean_tbl %>% select(case_id = `Participant ID`, aliquot_id = `Specimen Label`, tmt_plex)
) %>%
    mutate(
    ...
    select(preferred_sample_names, case_id, sample_type, aliquot_id, tmt_plex)
...
## Export
```{r}
selected_protein_se %>% saveRDS(here('final_output/CPTAC3_HNSCC_discovery/CPTAC3_HNSCC_umich_proteome.rds'))
selected_phospho_se %>% saveRDS(here('final_output/CPTAC3_HNSCC_discovery/CPTAC3_HNSCC_umich_phosphoproteome.rds'))
```

Export as GCT formats
```
g = cmapR::GCT(
  mat = assay(selected_protein_se),
  rdesc = rowData(selected_protein_se) %>% as.data.frame(),
  cdesc = colData(selected_protein_se) %>% as.data.frame()
)

cmapR::write_gct(g, '~/Downloads/hnscc_protein.gct', appenddim = FALSE)

g = cmapR::GCT(
  mat = assay(selected_phospho_se),
  rdesc = rowData(selected_phospho_se) %>% as.data.frame(),
  cdesc = colData(selected_phospho_se) %>% as.data.frame()
)

cmapR::write_gct(g, '~/Downloads/hnscc_phospho.gct', appenddim = FALSE)
```
